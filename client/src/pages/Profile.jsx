...
import { useUpdateProfileMutation } from "../globalState/api/authApi";
const Profile = () => {
  ...
  const onProfileSubmit = async (data) => {
    try {
      ...
      toast.success("Profile updated successfully!", TOAST_DESIGN_SUCCESS);
    } catch (error) {
      toast.error(
  ...
  const onPasswordSubmit = async (data) => {
    if (data.newPassword !== data.confirmPassword) {
      toast.error("New passwords do not match", TOAST_DESIGN_ERROR);
      return;
    }
    try {
      await updateProfile({
      ...
      toast.success("Password changed successfully!", TOAST_DESIGN_SUCCESS);
      reset();
    } catch (error) {
      toast.error(
        error.data.message || "An error occurred. Please try again.",
  ...
  const handleDisableAccount = () => {
    // toast.success('Account disabled successfully!', TOAST_DESIGN_SUCCESS);
  };

  const handleDeleteAccount = () => {
    // toast.error('Account deleted successfully!', TOAST_DESIGN_ERROR);
  };
    ...
    <>
      <div className="font-Lex bg-black/95 text-white min-h-screen mt-[10vh] px-4 sm:px-6 lg:px-12 ">
        <div className="max-w-4xl mx-auto">
          ...
          {activeTab === "settings" && (
            <div className="bg-[#0a141d]/60 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-xl hover:shadow-[#E11D48]/20 transition-all duration-300 animate-in fade-in duration-500">
              ...
              <form
                ...
                className="space-y-6"
              >
                <h2 className="text-xl sm:text-2xl font-bold text-[#E11D48] mb-4 text-shadow-sm">
                  Edit Profile
                </h2>
                <div>
                  <label
          ...
          {activeTab === "payments" && (
            <div className="bg-[#0a141d]/60 backdrop-blur-md border border-white/10 rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-xl hover:shadow-[#E11D48]/20 transition-all duration-300 animate-in fade-in duration-500">
              <h2 className="text-xl sm:text-2xl font-bold text-[#E11D48] mb-4 sm:mb-6 text-shadow-sm">
                Payment History
              </h2>
                ...
                <>
                  ...
                  <div className="md:hidden space-y-4">
                    {paymentHistory.map((payment, index) => (
                      <div
                        key={index}
                        className="bg-[#1a2634]/50 rounded-lg p-4 border border-white/10"
                      >
